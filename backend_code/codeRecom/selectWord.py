...
def init() :
    ...
    initList = ['var', 'int', 'uint', 'fixed', 'ufixed', 'string', 'byte', 'bytes', 'bool', 'address']
    intTypes = [
        'int%d' % m
    ...
    # merge multiple list
    builtInType = sum([intTypes, uintTypes, fixedTypes, ufixedTypes, stringTypes, bytesTypes], initList)
...
def select_word_init(context) :
    ...
    contract_user, func_user, var_user = build_user_dataset(context, builtInType)
