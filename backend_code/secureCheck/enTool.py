...
def deal_with_oyenteOut(contractName):
    ...
    for i in f_list:
        if os.path.splitext(i)[1] == '.json' and os.path.splitext(i)[
                0].find('.sol:') != -1:
...
def runTool_smartcheck(contractName, limitTime):
    ...
    ret1, seconds_passed1 = execute_command(
        "SmartCheck", cmd_smartcheck, timeout=limitTime)
    if ret1:
        print("Run SmartCheck Complete! cost time :", seconds_passed1)


def runTool_oyente(contractName, limitTime):
    cmd_oyente = 'sudo docker run -v $(pwd):/project qspprotocol/oyente-0.4.25 -s /project/' + \
        contractName + '.sol -j'
    ret2, seconds_passed2 = execute_command(
        "Oyente", cmd_oyente, timeout=limitTime)
    if ret2:
        print("Run Oyente Complete! cost time :", seconds_passed2)


def runTool_myth(contractName, limitTime):
    cmd_myth = 'sudo docker run -v $(pwd):/tmp mythril/myth analyze /tmp/' + \
        contractName + '.sol | tee mythReport.txt'
    ret3, seconds_passed3 = execute_command(
        "Mythril", cmd_myth, timeout=limitTime)
    if ret3:
        print("Run Mythril Complete! cost time :", seconds_passed3)


def runTool_securify(contractName, limitTime):
    cmd_securify = 'docker run -v $(pwd):/project qspprotocol/securify-usolc-0.5.3 -fs /project/' + \
        contractName + '.sol | tee securifyReport.json'
...
def runTools(contractName, limitTime, num):
    ...
    for i in range(num):
        t = threading.Thread(
            target=runTools_call, args=(
                contractName, limitTime, i,))
        threads.append(t)
...
if __name__ == '__main__':
    ...
    with open(CONTRACTFOLDER + contractName + '/finalReport.json', 'w') as f:
        ...
        lineList, backdoorList = detect_backdoor(
            CONTRACTFOLDER, contractName, limitTime)
        ...
        for i in range(1, maxIdx + 1):
            ...
            if len(vulMap[entID]) != 0 and getLevel(entID) != 'ignore':
                ...
                outJson['vulnerabilities'][entID]['description'] = getDescription(
                    entID)
