...
def dl_run(contractPath, fileName, timeout):
    ...
    cmd_bin = 'solc --bin-runtime ' + contractPath + fileName + \
        '/' + fileName + '.sol | tail -n 1 > ' + fileName + '.hex'
    print(cmd_bin)
    ...
    # run datalog
    cmd_dl = backdoorPath + 'MadMax/bin/analyze.sh ' + curPath + \
        fileName + '.hex ' + backdoorPath + 'backdoor.dl'
...
if __name__ == "__main__":
    ...
    lineList, backdoorList = detect_backdoor(
        ori_contractPath, fileName, timeout)
