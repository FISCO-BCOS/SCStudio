...
__VULNERABILITIES = {
    ...
    4: VulInfo(
        ...
        desc="The return value of a message call is not checked. Execution will resume even if the called contract throws an exception. If the call fails accidentally or an attacker forces the call to fail, this may cause unexpected behavior in the subsequent program logic.",
        swc="104",
    ...
    8: VulInfo(
        ...
        swc="110",
        advice="Consider whether the condition checked in the assert() is actually an invariant. If not, replace the assert() statement with a require() statement. If the exception is indeed caused by unexpected behavior of the code, fix the underlying bug(s) that allow the assertion to be violated.",
        level="error",
    ),
    9: VulInfo(
    ...
    48: VulInfo(
        ...
        desc="The readability of construction `if (condition) {revert();}` is worse than `require(condition);`",
        swc="",
        advice="Use `require(condition);`for better code readability.",
        level="warning",
    ),
    49: VulInfo(
        name="Using unitless number",
